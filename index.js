...
const vcard = 'BEGIN:VCARD\n' // Tarjeta de contacto
            ...
            + 'FN:Duda\n' // Nombre
            + 'ORG:Shanduy;\n' // Propietario
            + 'TEL;type=CELL;type=VOICE;waid=51901723418:+1 901 723 418\n' // ID de WhatsApp + número de teléfono
            + 'END:VCARD'
...
async function starts() {
	...
	client.on('open', () => {
		success('2', 'Conectado ValleBrite | 1.16.5')
	})
		...
		try {
			...
			mess = {
				...
				only: {
					...
					daftarB: `「NO ESTAS REGISTRADO」\n\nPA NO APARECES EN MI BASE DE DATOS ✋🥸🤚\n\nPara poder usarme escribe el siguente comando\n\nComando: ${prefix}daftar Nombre\nEjemplo: ${prefix}daftar daniel`,
				}
			}
    			const apakah = ['Si','No']
                        const kapankah = ['Otro día','Otra semana','Otro mes','Otro año']
			const botNumber = client.user.jid
			const ownerNumber = ["51901723418@s.whatsapp.net"] // replace this with your number
			...
			const isOwner = ownerNumber.includes(sender)
                        ...
                        const NomerOwner = '51901723418@s.whatsapp.net'
		...
		if (!isUser) return reply(mess.only.daftarB)
			...
			case 'todos':
				client.updatePresence(from, Presence.composing) 
					...
					mentions('╔══✪〘 *TODOS* 〙✪══\n╠➥'+teks+'╚═〘 _*ValleBrite | 1.16.5*_ 〙', members_id, true)
					break
                                case 'send':
					var pc = body.slice(6)
	...
	//SERVICIO DE MUSICA Y VIDEO 			
		...
		if (!isUser) return reply(mess.only.daftarB)
	        reply(mess.only.musica2)
                ...
                if (anu.error) return reply(anu.error)
                infomp3 = `*⌜Cancion Encontrada ✅⌟*\n◉ *Título:* ${anu.result.title}\n◉ *Fuente:* ${anu.result.source}\n◉ *Tamaño:* ${anu.result.size}\n\n*ESPERE ENVIANDO SU ARCHIVO MP3 ⚠*\n\n_*Servicio proveido por Luxor_`
                buffer = await getBuffer(anu.result.thumbnail)
                lagu = await getBuffer(anu.result.url_audio)
                client.sendMessage(from, buffer, image, {quoted: mek, caption: infomp3})
                client.sendMessage(from, lagu, audio, {mimetype: 'audio/mp4', filename: `${anu.title}.mp3`, quoted: mek})
